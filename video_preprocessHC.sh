...
# Config
video_info_path=$1 #"(..).csv"  # this file has the header:
...
# Stages:
STEP0=0 # downloads yt videos and trasncrptions

STEP1=0 # remove frames where there is no speech
...
if [ $STEP0 -eq "1" ]; then
  ...
  mkdir -p ${video_preproces_dir}/yt_transcriptions/
  mkdir -p ${video_preproces_dir}/yt_videos/



  while IFS=, read -r keyword videoID channel_id diagnosis gender age role category speakerID ti tf c
  do
...
if [ $STEP4 -eq "1" ]; then
  ...
  # necessary to add header to APD.csv. To run this step we need to do sh video_preprocess.sh APD.csv
  ...
  python3 ${pre_process_scripts_dir}/utils/make_data_csv.py \
                           ...
                           --output_csv ${root}/data_info/dataHC.csv
fi
...
if [ $STEP5 -eq "1" ]; then
  ...
  python3 ${pre_process_scripts_dir}/utils/detect_faces_video.py \
                                --data_info ${root}/data_info/dataHC.csv \
...
if [ $STEP6 -eq "1" ]; then
  ...
  python3 ${pre_process_scripts_dir}/lip_readings/crop_mouth_from_video.py \
      ...
      --filename-path ${root}/data_info/dataHC_only_faces.csv \
...
if [ $STEP7 -eq "1" ]; then
  ...
  } < ${root}/data_info/dataHC_only_faces.csv

  # Update data.csv with roi paths and embedding paths
 awk -F',' -v root=$root 'NR==1 {print $0 ",mouth_roi_path,lipread_emb_path"} NR>1 {print $0 "," root "/data/mouth_rois/" $2 "/" $1 ".npz," root "/features/lipread_emb/"  $2 "/" $1 ".npz,"}' ${root}/data_info/dataHC_only_faces.csv > tmp
 mv tmp ${root}/data_info/dataHC_only_faces.csv
