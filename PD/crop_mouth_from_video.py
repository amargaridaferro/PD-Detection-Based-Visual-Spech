...
"""
Comments by mctb:
    1. interpolates landmarks
    2. reads each video and corresponding landmarks, crops ROIs
    3. converts to gray scale
    4. saves each video, now represented by a sequence of cropped frames to one .npz

both video-direc and landmark-direc should contain the videos and landmarks with the same
names, which are listed in filename-path, together with subject id.

videos should be .mp4, landmarks should be .npz.

cropping function alligns faces with mean-face file, and crops with 96x96.
"""
...
def load_args(default_config=None):
    ...
    # -- mean face utils
    parser.add_argument('--mean-face', default='Tese/PD/pre_process_scripts/lip_readings/20words_mean_face.npy', help='mean face pathname')
    # -- mouthROIs utils
...
# BEGIN
# reads filename_path which has the list of all videos and corresponding subject ids
...
lines_df = pd.read_csv(args.filename_path)
filenames_ = lines_df.uid.values
subjects_ = lines_df.subject_id.values

#for filename_idx, line in enumerate(lines):
for filename_idx, (filename, person_id) in enumerate(zip(filenames_, subjects_)):
    ...
    video_pathname = args.video_direc + '/' + person_id + '/' + filename+'.mp4'
    landmarks_pathname = args.landmark_direc + '/' + person_id + '/' + filename+'.npy'
    dst_pathname = args.save_direc + '/' + person_id + '/' + filename+'.npz'
    ...
    if os.path.exists(dst_pathname):
        print ('Mouth roi for file ', filename, ' already exists. Skipping DDDDDDDDDDD.')
        continue
    ...
    if sequence is not None:
        ...
        print("cannot crop from {}.".format(filename), 'sequence is None.')
print('Done.')
